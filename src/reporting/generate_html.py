...
def generate_html(usernames, report_variables, report_chart_path, PROJ_ROOT):
	resources_file_path = os.path.join(PROJ_ROOT,
									"references",
									"report_materials")
	interim_report_filepath = os.path.join(PROJ_ROOT,
									"reports",
									"interim_reports")
	...
	template = env.get_template('HeyCharlieReportTemplateV3.html')
	...
	for username in usernames:
		...
		filename = username + " HeyCharlieReport " + report_variables[username]['report_date']
		html_filename = filename + ".html"
		pdf_filename = filename + ".pdf"

		html_file_path = os.path.join(interim_report_filepath, html_filename)
		html_file = open(html_file_path, "w")
		html_file.write(html_out)
		html_file.close()

		convert_to_pdf.convert_html_to_pdf(pdf_filename, html_out, PROJ_ROOT)
